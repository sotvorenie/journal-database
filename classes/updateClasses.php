...
if ($_SERVER['REQUEST_METHOD'] === 'PATCH') {
    ...
    $sql = "UPDATE classes SET first_lesson = ?, second_lesson = ?, third_lesson = ?, fourth_lesson = ?, fifth_lesson = ?
               WHERE user_id = ? AND group_id = ? AND student_id = ? AND date_info = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssiiis", $first_lesson, $second_lesson, $third_lesson, $fourth_lesson, $fifth_lesson, $user_id, $group_id, $student_id, $date_info);

    if ($stmt->execute()) {
        echo json_encode(['success' => true, 'message' => 'Данные о расписании успешно обновлены!!']);

        //создаем редактированную копию записи с обновленными ячейками журнала
