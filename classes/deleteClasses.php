...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $sql = "DELETE FROM classes WHERE user_id = ? AND group_id = ? AND date_info = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iis", $user_id, $group_id, $date_info);

    if ($stmt->execute()) {
        echo json_encode(['success' => true, 'message' => 'Расписание удалено успешно!!']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Расписание не было удалено, причина: ' . $stmt->error]);
    }
} else {
